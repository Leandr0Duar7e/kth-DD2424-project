...
def evaluate_model(model, test_loader, criterion, device, binary_classification):
    ...
    with torch.no_grad():
        for inputs, labels in test_loader:
            ...
            predicted = (torch.sigmoid(outputs) > 0.5).float() if binary_classification else outputs.argmax(dim=1)
...
def train_model_with_adam(model, device, train_loader, val_loader, binary_classification, num_epochs=10, learning_rate=0.001):
